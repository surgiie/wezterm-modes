...
---
--- The command is delivered through the window's action queue (SendString /
--- SendKey) rather than pane:send_text. send_text writes to the pty
--- synchronously, so when invoked from a key-table callback it races the
--- keystroke that triggered the callback and the key-table pop — typing ":x"
--- fast enough to batch both key events would swallow the command's first
--- byte. Queued actions are ordered strictly after the current key event and
--- the ClearKeyTableStack below.
---
--- That ordering still isn't enough on its own: WezTerm reliably drops one
--- leading byte of the injected input when the triggering keystroke and the
--- send land in the same input batch. So the payload is prefixed with a
--- throwaway SPACE — if it's dropped the real command is untouched, and if it
--- survives a leading space is inert to the shell (` cmd` runs like `cmd`).
--- The space is omitted for the type-only (`execute = false`) path, which
--- instead absorbs the drop with a leading CTRL+B (a no-op at column 0).
--- @param window  table  WezTerm Window object
--- @param pane    table  WezTerm Pane object
