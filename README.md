...
vim_modes.apply_to_config(config, {
    ...
    -- called on every status tick; its return value (a wezterm.format-encoded
    -- string, or plain text) is prepended to whatever this plugin renders in
    -- the left status. Lets you decorate the status bar with your own
    -- app-specific state without this plugin needing to know anything about
    -- it - e.g. reflecting some other program's state via a WezTerm user var:
